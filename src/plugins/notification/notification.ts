...
export default class Notification {
  ...
  private $root: any;
  private eventBus: Events<NotificationOptions> =
    new Events<NotificationOptions>();
  constructor() {
  ...
  success(message: string, title = ""): void {
    this.open(
      new NotificationOptions(title, message, EnumNotificationType.Success)
    );
  }
  error(message: string, title = "Ошибка"): void {
    this.open(
      new NotificationOptions(title, message, EnumNotificationType.Error)
    );
  }
  ...
  mount = (
    component,
    { props = {}, children = {}, element = null, app = null } = {}
  ) => {
    console.log("MOUNT");
